...
class EpicMiddleware<State> extends MiddlewareClass<State> {
  ...
  final StreamController<Epic<State>> _epics = StreamController.broadcast(sync: true);
  ...
  void call(Store<State> store, dynamic action, NextDispatcher next) {
    if (!_isSubscribed) {
      _epics.stream.switchMap<dynamic>((epic) => epic(_actions.stream, EpicStore(store))).listen(store.dispatch);
